...
Starting DEVD claims its configured directory with a durable ownership marker and one live PID-scoped lock before serving mutations. A surviving live owner is rejected, while a lock from a dead process is recovered safely. Client resource commands serialize local mutations so each request carries the revision returned by its predecessor. CLI template, inventory, Agent Import, and inventory-print commands never open the business data directory; they send the same HTTP-shaped request semantics through a required named IPC instance. Unix sockets are used on macOS/Linux and Named Pipes on Windows. `--data-dir`, `--devd-url`, and `TUCKMARK_DEVD_URL` are recognized only to return a migration error. DEVD data and Agent Import HTTP routes accept only requests whose connected peer is loopback; `Host` and `Origin` headers never establish local trust for a non-loopback client.
...
- CLI catalog, inventory, Agent Import, and inventory-print commands read and mutate DEVD data through `--instance` or `TUCKMARK_DEVD_INSTANCE`; missing instances and legacy URL/directory flags fail without fallback.
