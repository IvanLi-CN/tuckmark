...
test("browser-static build registers a service worker and works offline after first load", async ({
  ...
  await expect
    .poll(
      async () =>
        page.locator(".tm-shell").getAttribute("data-offline-warmup-status"),
      {
        timeout: 15_000,
      },
    )
...
test("browser-static build ships complete PWA assets without remote font dependency", async () => {
  ...
  expect(
    assetFiles.some(
      (file) => file.includes("feature-runtime") && file.endsWith(".css"),
    ),
  ).toBe(true);
  ...
  expect(
    manifest.icons.some((icon) => icon.src === "./pwa/tuckmark-icon-192.png"),
  ).toBe(true);
  expect(
    manifest.icons.every((icon) => icon.purpose?.includes("maskable")),
  ).toBe(true);
  ...
  expect(serviceWorker).toContain(
    "requestUrl.pathname.endsWith(VERSION_METADATA_URL.slice(1))",
  );
