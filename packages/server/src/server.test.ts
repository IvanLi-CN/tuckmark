...
class FakeServerService implements ServerService {
  ...
  async getArtifact(artifactId: string): Promise<Awaited<ReturnType<TuckmarkService["getArtifact"]>>> {
...
describe("server", () => {
  ...
  it("returns a clear error when server-side print is explicitly disabled", async () => {
    ...
    service.printByArtifact = async () => {
      throw new Error("Server-side printer control is disabled. Set TUCKMARK_ENABLE_SERVER_SIDE_PRINT=1 to enable it.");
    };
    const app = createApp(service);
    const server = app.listen(0);
    const baseUrl = await new Promise<string>((resolve, reject) => {
