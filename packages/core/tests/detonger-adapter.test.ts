...
afterEach(async () => {
  ...
  delete process.env.TUCKMARK_DETONGER_PACKET_ENCODER;
  delete process.env.TUCKMARK_DETONGER_COMMAND;
  delete process.env.TUCKMARK_DETONGER_PRINT_TIMEOUT_MS;
  delete process.env.TUCKMARK_DETONGER_PNG_ROWS_PER_CHUNK;
  vi.restoreAllMocks();
});
...
describe("DetongerAdapter", () => {
  ...
  it("surfaces detonger json errors without retrying hardware print", async () => {
    ...
    vi.spyOn(directAdapter as DetongerAdapter, "detongerArgs" as never).mockImplementation(
      ((args: string[]) => [fakeDetongerPath, ...args]) as never
    );

    await expect(
      directAdapter.printArtifact(printerId, preview.artifact)
    ).rejects.toThrow(/detonger print job failed: \[timeout\] timeout/);
  ...
  it("times out hanging detonger print commands and writes a command log", async () => {
    ...
    await expect(adapter.printArtifact(printerId, preview.artifact)).rejects.toThrow(/timed out after 200ms/);
