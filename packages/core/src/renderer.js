...
function pixelIsBlack(png, x, y, threshold) {
...
function setMonoPixel(png, x, y, isBlack) {
    const idx = (png.width * y + x) << 2;
...
function normalizeContinuousPaperPng(pngBuffer, renderOptions) {
    ...
    for (let y = 0; y < png.height; y += 1) {
        const row = Array.from({ length: png.width }, (_, x) => pixelIsBlack(png, x, y, renderOptions.threshold));
        let darkBits = row.reduce((sum, bit) => sum + Number(bit), 0);
        if (darkBits <= continuousSafetyRowDensityThreshold) {
            continue;
        }
        const protectedDots = new Array(png.width).fill(false);
        let runStart = -1;
    ...
    return mutated
        ? Buffer.from(PNG.sync.write(png))
        : pngBuffer;
}
