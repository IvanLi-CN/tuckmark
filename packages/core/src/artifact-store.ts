...
import { type PreviewArtifact, previewArtifactSchema } from "./types.js"
...
export class ArtifactStore {
  ...
  async writeArtifact(
    ...
    files: { png: Buffer; bitmap: Buffer; svg: string }
  ...
  async listArtifacts(): Promise<PreviewArtifact[]> {
    ...
    const artifacts = await Promise.all(
      entries.filter((entry) => entry.isDirectory()).map((entry) => this.getArtifact(entry.name))
    )
