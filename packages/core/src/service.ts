...
export class TuckmarkService {
  ...
  private normalizeTemplateInput(template: TemplateDefinition, input: Record<string, string>): Record<string, string> {
  ...
  private resolveRequestedPrinter(
    ...
    throw new Error(`Printer is no longer available: ${request.printerId}${suffix}. Refresh printers and retry.`);
  }

  private async resolvePrintableRequest(
    request: { printerId: string; printerName?: string | undefined }
  ): Promise<Printer> {
  ...
  async printByTemplate(request: PrintByTemplateRequest): Promise<{ preview: PreviewResult; job: PrintJob }> {
    this.ensureServerSidePrintEnabled();
  ...
  async printCanvas(request: PrintCanvasRequest): Promise<{ preview: PreviewResult; job: PrintJob }> {
    this.ensureServerSidePrintEnabled();
