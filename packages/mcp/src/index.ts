...
import { safeTextLabelSchema, TuckmarkService, type TuckmarkService as TuckmarkServiceType } from "@tuckmark/core";
...
export function registerServer(server: McpServer, service: McpService = new TuckmarkService()): McpServer {
  server.registerTool(
    "list_printers",
    {
      title: "List Printers",
  ...
  async ({ printerId, printerName, templateId, input, renderOptions }) => {
    const result = await service.printByTemplate({ printerId, printerName, templateId, input, renderOptions });
    return {
      content: [{ type: "text", text: JSON.stringify(result, null, 2) }],
      structuredContent: result
    };
  ...
  server.registerResource(
    ...
    { title: "Tuckmark Artifact", description: "Stored preview artifact", mimeType: "application/json" },
    async (uri, { artifactId }) => {
      ...
      return {
        contents: [{ uri: uri.href, text: JSON.stringify(artifact, null, 2), mimeType: "application/json" }]
      };
    }
  );
  ...
  server.registerResource(
    ...
    async (uri, { artifactId }) => {
      ...
      return {
        contents: [{ uri: uri.href, text: JSON.stringify(packets, null, 2), mimeType: "application/json" }]
      };
    }
  );
