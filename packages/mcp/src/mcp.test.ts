...
describe("mcp", () => {
  it("registers template preview and artifact print tools on the shared service", async () => {
    ...
    const registeredTools = (server as unknown as { _registeredTools: Record<string, { executor: (...args: unknown[]) => Promise<unknown> }> })
      ._registeredTools;
    ...
    expect((preview as { structuredContent: { artifact: { id: string } } }).structuredContent.artifact.id).toBe(
      "artifact-1"
    );
    ...
    expect((job as { structuredContent: { status: string } }).structuredContent.status).toBe("completed");
    ...
    expect((probe as { structuredContent: { ok: boolean; stage: string } }).structuredContent.ok).toBe(true);
    expect((probe as { structuredContent: { ok: boolean; stage: string } }).structuredContent.stage).toBe("complete");
    ...
    const resourceTemplates = (
      server as unknown as {
        _registeredResourceTemplates: Record<
          string,
          {
            resourceTemplate: { listCallback?: (ctx: unknown) => Promise<{ resources: Array<{ uri: string }> }> };
            readCallback: (uri: URL, vars: Record<string, string>, ctx: unknown) => Promise<{ contents: Array<{ text?: string }> }>;
          }
        >;
      }
    ...
    expect(artifactList?.resources.map((resource) => resource.uri)).toContain("tuckmark://artifacts/artifact-1");
    expect(pngList?.resources.map((resource) => resource.uri)).toContain("tuckmark://artifacts/artifact-1/png");
