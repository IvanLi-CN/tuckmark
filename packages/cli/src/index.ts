...
async function handlePreview(args: string[]): Promise<void> {
  ...
  if (templateArg) {
    ...
    const payload = templateInputSchema.parse({ templateId: templateArg, input: JSON.parse(inputJson) });
  ...
  if (canvasArg) {
    ...
    const preview = await service.previewCanvas({
      canvas: {
        ...
        elements: [
          { kind: "rect", x: 6, y: 6, width: canvas.width - 12, height: canvas.height - 12, strokeWidth: 2, fill: "white", stroke: "#111111", radius: 8 },
          { kind: "text", key: "body", value: canvas.text, x: 18, y: 48, fontSize: 22, fontWeight: "normal", align: "left", width: canvas.width - 36, maxLines: 8 }
        ]
      },
      renderOptions: parseRenderOptions(args)
    });
...
async function handleBatchPreview(args: string[]): Promise<void> {
  ...
  const result = await service.previewBatch({ templateId, csvText, renderOptions: parseRenderOptions(args) });
...
async function handlePrint(args: string[]): Promise<void> {
  ...
  if (artifactId) {
    console.log(JSON.stringify(await service.printByArtifact({ printerId, printerName, artifactId }), null, 2));
    return;
  }
  if (artifactIdsArg) {
    const artifactIds = z.array(z.string().min(1)).parse(JSON.parse(artifactIdsArg));
    console.log(JSON.stringify(await service.printBatch({ printerId, printerName, artifactIds }), null, 2));
    return;
  }
  if (safeTextArg) {
    const payload = safeTextSchema.parse(JSON.parse(safeTextArg));
    console.log(
      JSON.stringify(
        await service.printSafeTextLabel(printerId, {
          ...payload,
          renderOptions: parseRenderOptions(args)
        }, printerName),
        null,
